...
use App\Http\Controllers\Backend\MoviemanageController;
...
Route::group([
    ...
    Route::group([
        ...
        Route::get('/', [MoviemanageController::class, 'index'])
            ->name('index')
            ->breadcrumbs(function (Trail $trail) {
                $trail->parent('admin.dashboard')
                    ->push(__('Movie Management'), route('admin.auth.role.index'));
            });

        Route::get('create', [MoviemanageController::class, 'create'])
            ->name('create')
            ->breadcrumbs(function (Trail $trail) {
                $trail->parent('admin.auth.role.index')
                    ->push(__('Create Movie'), route('admin.auth.movie.create'));
            });

        Route::post('/', [MoviemanageController::class, 'store'])->name('store');

        Route::group(['prefix' => '{movie}'], function () {
            

            Route::get('/', [MoviemanageController::class, 'show'])
            ->name('show')
            ->middleware('permission:admin.access.user.list')
            ->breadcrumbs(function (Trail $trail, MovieListing $user) {
                $trail->parent('admin.auth.movie.index')
                    ->push($user->name, route('admin.auth.movie.show', $user));
            });

            Route::get('edit', [MoviemanageController::class, 'edit'])
                ->name('edit')
                ->breadcrumbs(function (Trail $trail, MovieListing $user) {
                    $trail->parent('admin.auth.movie.index')
                        ->push(__('Editing :role', ['role' => $user->name]), route('admin.auth.movie.edit', $user));
                });

            Route::patch('/', [MoviemanageController::class, 'update'])->name('update');
            Route::delete('/', [MoviemanageController::class, 'destroy'])->name('destroy');
        });
    });
});
